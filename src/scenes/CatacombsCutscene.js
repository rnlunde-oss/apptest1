// Catacombs Cutscene — triggered when party approaches the Catacombs
// Uses black placeholder images (to be replaced with art later)

export class CatacombsCutscene extends Phaser.Scene {
  constructor() {
    super('CatacombsCutscene');
  }
  ...
  create() {
    ...
    const lines = [
      { text: 'The road to the Catacombs bent northwards until the final grove came into view. The stone archway ahead was flanked on either side by skeletal watchmen. This was not a staging ground, this was a garrison.', image: null },
      { text: 'Metz: "Have your wits about you. This is going to be a fight."', image: null },
      { text: 'Rivin: "Wouldn\'t have it any other way."', image: null },
      { text: 'Rickets\' eyes lit and he let out a small giggle of kindled excitement.', image: null },
    ];
    ...
    const showLine = () => {
      if (idx >= lines.length) {
        ...
        this.cameras.main.once('camerafadeoutcomplete', () => {
          const savedPos = this.registry.get('catacombsCutscenePlayerPos');
          this.scene.start('Overworld', { fromCatacombsCutscene: true, playerPos: savedPos });
        });
        return;
      }
