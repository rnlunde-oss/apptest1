// Bracken Cutscene — triggered when player approaches Fort Bracken
// Uses black placeholder images (to be replaced with art later)

export class BrackenCutscene extends Phaser.Scene {
  constructor() {
    super('BrackenCutscene');
  }
  ...
  create() {
    ...
    const lines = [
      { text: 'The sight of Bracken came into view. True to the rumors, fire and smoke were visible from beyond the town palisades. While it had been built for trade, it was not known to be well-garrisoned.', image: null },
      { text: 'Metz: "The town is taken. We will need to progress with both speed and caution."', image: null },
      { text: 'Lyra: "Lead the way, captain."', image: null },
    ];
    ...
    const showLine = () => {
      if (idx >= lines.length) {
        ...
        this.cameras.main.once('camerafadeoutcomplete', () => {
          const savedPos = this.registry.get('brackenCutscenePlayerPos');
          this.scene.start('Overworld', { fromBrackenCutscene: true, playerPos: savedPos });
        });
        return;
      }
